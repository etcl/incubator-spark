...
class Client(conf: Configuration, args: ClientArguments) extends YarnClientImpl with Logging {
  ...
  def run() {
    ...
    val newApp = this.createApplication()
    val appId = newApp.getApplicationSubmissionContext.getApplicationId

    val appContext = createApplicationSubmissionContext(appId)
    val localResources = prepareLocalResources(appId, ".sparkStaging")
    val env = setupLaunchEnv(localResources)
    val amContainer = createContainerLaunchContext(newApp.getApplicationSubmissionContext, localResources, env)
  ...
  def createContainerLaunchContext(newApp: ApplicationSubmissionContext,
                                   localResources: HashMap[String, LocalResource],
